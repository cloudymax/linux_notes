...
--verbose \
--progress \
-p -b -e 'ssh -T -c aes128-gcm@openssh.com -o Compression=no -x' \
...
rsync --times \
  ...
  --verbose \
  --progress \
  -p -b -e '/usr/bin/ssh -T -c aes128-gcm@openssh.com -o Compression=no -x' \
  $USER@$HOST:$REMOTE_DIR/$FILE $LOCAL_DIR
